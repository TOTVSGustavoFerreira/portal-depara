    ...
    function normalizeDatabaseProperties(db) {
      ...
      if (db.DADOS_RM_SECOES && Array.isArray(db.DADOS_RM_SECOES)) {
        db.DADOS_RM_SECOES.forEach(item => {
          ...
          const sec = String(item.COD_SECAO !== undefined ? item.COD_SECAO : (item.cod_secao !== undefined ? item.cod_secao : "")).trim();
          item.COD_SECAO = sec;
          item.cod_secao = sec;
          ...
          const set = String(item.SETOR !== undefined ? item.SETOR : (item.setor !== undefined ? item.setor : "")).trim();
          item.SETOR = set;
          item.setor = set;
        });
      }
  ...
  },
  {
    ...
    const OFFICIAL_HEADERS = {
      ...
      DADOS_RM_SECOES: ["COLIGADA", "FILIAL", "COD_SECAO", "DESCRICAO", "SETOR"]
    };
    ...
    function updateDashboard() {
      ...
      if (currentModule === 'situacao') {
        ...
        // Contar quantos motivos criados manualmente existem EM USO nesta aba
        let manualCount = 0;
        if (database) {
    ...
    function applyFilters() {
      ...
      filteredData = database.filter(item => {
        ...
        if (activeKpiFilter === 'manual_motives') {
          if (currentModule !== 'situacao') return false;
    ...
    function getSecaoName(coligada, filial, cod) {
      ...
      const item = rawJsonDatabase.DADOS_RM_SECOES.find(s => 
        ...
        String(s.COD_SECAO).trim() === codStr
      );
      return item ? item.DESCRICAO : '';
    }

    function getSecoesOptionsHtml(currentVal) {
      let html = `<option value="">-- Selecione a Seção --</option>`;
      if (!rawJsonDatabase || !rawJsonDatabase.DADOS_RM_SECOES) return html;
      
      const secoes = rawJsonDatabase.DADOS_RM_SECOES;
      
      const parentCodes = new Set();
      secoes.forEach(s => {
        const parts = String(s.COD_SECAO).split('.');
        let prefix = "";
      ...
      secoes.forEach(s => {
        const key = `${s.COLIGADA}_${s.FILIAL}_${s.COD_SECAO}`;
        const isParent = parentCodes.has(key);
        const optionValue = `${s.COLIGADA}|${s.FILIAL}|${s.COD_SECAO}`;
        const isSelected = (optionValue === currentVal) ? "selected" : "";
        const cnpjText = s.CNPJ ? ` (CNPJ: ${s.CNPJ})` : '';
        const label = `[Col: ${s.COLIGADA} | Fil: ${s.FILIAL}] ${s.COD_SECAO} - ${s.DESCRICAO}${cnpjText}`;
      ...
      if (currentVal && currentVal.includes('|')) {
         ...
         const exists = secoes.some(s => `${s.COLIGADA}_${s.FILIAL}_${s.COD_SECAO}` === fallbackKey);
         if (!exists) {
    ...
    function renderGenericTableContent(tbody, itemsToRender) {
      itemsToRender.forEach(item => {
        ...
        if (isEditing) {
          if (currentModule === 'coligadas') {
            ...
            trHtml = `
              ...
              <td style="overflow: visible;">
                <select id="edit-CODIGO_PARA-${item.rowNum}" class="input-field" style="padding: 6px; font-size: 0.8rem; width: 100%;">
                   ${getSecoesOptionsHtml(currentVal)}
                </select>
              </td>
              <td class="col-acoes">
                <div class="action-icons">
        ...
        if (isExpanded) {
          ...
          let colsCount = 6;
          if (currentModule === 'funcoes' || currentModule === 'sindicatos' || currentModule === 'secoes') colsCount = 7;
          ...
          if (currentModule === 'funcoes') {
            ...
            if (isEditing) {
              detailHtml = `
                <div style="font-family: 'Outfit', sans-serif; font-size: 0.85rem; display: flex; flex-direction: column; gap: 8px; width: 100%; max-width: 650px; padding: 12px; background: #f8fafc; border: 1px dashed #cbd5e1; border-radius: 8px;">
              ...
              detailHtml = `
                <div style="font-family: 'Outfit', sans-serif; font-size: 0.85rem; padding: 12px; background: #f8fafc; border-radius: 8px; border: 1px solid #f1f5f9; width: 100%; max-width: 650px;">
                  ${obsLabel}
                </div>
              `;
            }
    ...
    function saveRowEdition(rowNum) {
      ...
      if (currentModule === 'eventos') {
        ...
        updatedData = { CODCOLIGADA: codColigada, CODFILIAL_PARA: codFilialPara };
